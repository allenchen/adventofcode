...
class Board():
    ...
    def trailhead_score(self, trailhead):
        # bfs
        visit_queue = [[trailhead] + [n] for n in self.find_valid_neighbors(trailhead)]
        ...
        while len(visit_queue) > 0:
            ...
            path = visit_node[:-1]
            true_node = visit_node[-1]

            if self.get(true_node) == "9":
                score += [visit_node]
            print(path)
            print(visit_queue)

            visit_queue += [path + [n] for n in self.find_valid_neighbors(true_node)]
        return len(score)
